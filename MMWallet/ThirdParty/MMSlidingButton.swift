...
@IBDesignable class MMSlidingButton: UIView{
    ...
    func setSuccesIcon() {
        let succesIcon = UIImageView(image: R.image.checkArrowIcon())

        activityIndicator.removeFromSuperview()

        dragPoint.addSubview(succesIcon)
        succesIcon.snp.makeConstraints { (make) -> Void in
            make.center.equalTo(self.dragPoint)
        }
    ...
    func setUpButton(){
        ...
        if !self.buttonText.isEmpty{
            ...
            self.buttonLabel               = UILabel(frame: CGRect(x: dragPointWidth/2, y: 0, width: self.frame.size.width, height: self.frame.size.height))
    ...
    func unlock(){
        UIView.transition(with: self, duration: 0.2, options: .curveEaseOut, animations: {
            ...
            if Status{
                ...
                self.dragPoint.addSubview(self.activityIndicator)

//
//  TransactionCommentedCell.swift
//  MMWallet
//
...
class TransactionCommentedCell: UITableViewCell {

    var delegate: TransactionCommentedCellDelegate?
    ...
    override func awakeFromNib() {
        ...
        descriptionView.layer.shadowOffset = CGSize(width: 0.0, height: 0.2)
        descriptionView.layer.shadowRadius = 2
        descriptionView.layer.shadowOpacity = 0.05
        //        containerView.layer.masksToBounds = false
        let path = UIBezierPath(rect: CGRect(x: 0, y: 0, width: descriptionView.frame.width, height: descriptionView.frame.height))
        descriptionView.layer.shadowPath = path.cgPath
    }
        ...
        didSet {
            if let transactionModel = transactionModel {
                ...
                if transactionModel.fromDirection!.name.isEmpty {
                    ...
                    attachment.image = R.image.contact_add()
                ...
                descriptionView.backgroundColor = UIColor(componentType: .textFieldBackground)
                commentLabel.textColor = UIColor(componentType: .textFieldText)
                commentLabel.text = transactionModel.descr
    ...
    @objc func handleDirectionFromTap(_ sender: UITapGestureRecognizer) {
        ...
        delegate?.transactionCommentedCellAddContactDirectionFrom(self, fromTransaction: transactionModel.id)
    }

    @objc func handleDirectionToTap(_ sender: UITapGestureRecognizer) {
        guard let transactionModel = transactionModel else { return }

        delegate?.transactionCommentedCellAddContactDirectionTo(self, fromTransaction: transactionModel.id)
    }
}

protocol TransactionCommentedCellDelegate {
